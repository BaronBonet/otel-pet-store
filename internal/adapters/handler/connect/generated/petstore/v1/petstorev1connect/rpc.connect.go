// Code generated by protoc-gen-connect-go. DO NOT EDIT.
//
// Source: petstore/v1/rpc.proto

package petstorev1connect

import (
	connect "connectrpc.com/connect"
	context "context"
	errors "errors"
	v1 "github.com/BaronBonet/otel-pet-store/internal/adapters/handler/connect/generated/petstore/v1"
	http "net/http"
	strings "strings"
)

// This is a compile-time assertion to ensure that this generated file and the connect package are
// compatible. If you get a compiler error that this constant is not defined, this code was
// generated with a version of connect newer than the one compiled into your binary. You can fix the
// problem by either regenerating this code with an older version of connect or updating the connect
// version compiled into your binary.
const _ = connect.IsAtLeastVersion1_13_0

const (
	// PetStoreServiceName is the fully-qualified name of the PetStoreService service.
	PetStoreServiceName = "petstore.v1.PetStoreService"
)

// These constants are the fully-qualified names of the RPCs defined in this package. They're
// exposed at runtime as Spec.Procedure and as the final two segments of the HTTP route.
//
// Note that these are different from the fully-qualified method names used by
// google.golang.org/protobuf/reflect/protoreflect. To convert from these constants to
// reflection-formatted method names, remove the leading slash and convert the remaining slash to a
// period.
const (
	// PetStoreServiceCreatePetProcedure is the fully-qualified name of the PetStoreService's CreatePet
	// RPC.
	PetStoreServiceCreatePetProcedure = "/petstore.v1.PetStoreService/CreatePet"
	// PetStoreServiceGetPetProcedure is the fully-qualified name of the PetStoreService's GetPet RPC.
	PetStoreServiceGetPetProcedure = "/petstore.v1.PetStoreService/GetPet"
	// PetStoreServiceListPetsProcedure is the fully-qualified name of the PetStoreService's ListPets
	// RPC.
	PetStoreServiceListPetsProcedure = "/petstore.v1.PetStoreService/ListPets"
	// PetStoreServiceUpdatePetStatusProcedure is the fully-qualified name of the PetStoreService's
	// UpdatePetStatus RPC.
	PetStoreServiceUpdatePetStatusProcedure = "/petstore.v1.PetStoreService/UpdatePetStatus"
)

// PetStoreServiceClient is a client for the petstore.v1.PetStoreService service.
type PetStoreServiceClient interface {
	CreatePet(context.Context, *connect.Request[v1.CreatePetRequest]) (*connect.Response[v1.CreatePetResponse], error)
	GetPet(context.Context, *connect.Request[v1.GetPetRequest]) (*connect.Response[v1.GetPetResponse], error)
	ListPets(context.Context, *connect.Request[v1.ListPetsRequest]) (*connect.Response[v1.ListPetsResponse], error)
	UpdatePetStatus(context.Context, *connect.Request[v1.UpdatePetStatusRequest]) (*connect.Response[v1.UpdatePetStatusResponse], error)
}

// NewPetStoreServiceClient constructs a client for the petstore.v1.PetStoreService service. By
// default, it uses the Connect protocol with the binary Protobuf Codec, asks for gzipped responses,
// and sends uncompressed requests. To use the gRPC or gRPC-Web protocols, supply the
// connect.WithGRPC() or connect.WithGRPCWeb() options.
//
// The URL supplied here should be the base URL for the Connect or gRPC server (for example,
// http://api.acme.com or https://acme.com/grpc).
func NewPetStoreServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) PetStoreServiceClient {
	baseURL = strings.TrimRight(baseURL, "/")
	petStoreServiceMethods := v1.File_petstore_v1_rpc_proto.Services().ByName("PetStoreService").Methods()
	return &petStoreServiceClient{
		createPet: connect.NewClient[v1.CreatePetRequest, v1.CreatePetResponse](
			httpClient,
			baseURL+PetStoreServiceCreatePetProcedure,
			connect.WithSchema(petStoreServiceMethods.ByName("CreatePet")),
			connect.WithClientOptions(opts...),
		),
		getPet: connect.NewClient[v1.GetPetRequest, v1.GetPetResponse](
			httpClient,
			baseURL+PetStoreServiceGetPetProcedure,
			connect.WithSchema(petStoreServiceMethods.ByName("GetPet")),
			connect.WithClientOptions(opts...),
		),
		listPets: connect.NewClient[v1.ListPetsRequest, v1.ListPetsResponse](
			httpClient,
			baseURL+PetStoreServiceListPetsProcedure,
			connect.WithSchema(petStoreServiceMethods.ByName("ListPets")),
			connect.WithClientOptions(opts...),
		),
		updatePetStatus: connect.NewClient[v1.UpdatePetStatusRequest, v1.UpdatePetStatusResponse](
			httpClient,
			baseURL+PetStoreServiceUpdatePetStatusProcedure,
			connect.WithSchema(petStoreServiceMethods.ByName("UpdatePetStatus")),
			connect.WithClientOptions(opts...),
		),
	}
}

// petStoreServiceClient implements PetStoreServiceClient.
type petStoreServiceClient struct {
	createPet       *connect.Client[v1.CreatePetRequest, v1.CreatePetResponse]
	getPet          *connect.Client[v1.GetPetRequest, v1.GetPetResponse]
	listPets        *connect.Client[v1.ListPetsRequest, v1.ListPetsResponse]
	updatePetStatus *connect.Client[v1.UpdatePetStatusRequest, v1.UpdatePetStatusResponse]
}

// CreatePet calls petstore.v1.PetStoreService.CreatePet.
func (c *petStoreServiceClient) CreatePet(ctx context.Context, req *connect.Request[v1.CreatePetRequest]) (*connect.Response[v1.CreatePetResponse], error) {
	return c.createPet.CallUnary(ctx, req)
}

// GetPet calls petstore.v1.PetStoreService.GetPet.
func (c *petStoreServiceClient) GetPet(ctx context.Context, req *connect.Request[v1.GetPetRequest]) (*connect.Response[v1.GetPetResponse], error) {
	return c.getPet.CallUnary(ctx, req)
}

// ListPets calls petstore.v1.PetStoreService.ListPets.
func (c *petStoreServiceClient) ListPets(ctx context.Context, req *connect.Request[v1.ListPetsRequest]) (*connect.Response[v1.ListPetsResponse], error) {
	return c.listPets.CallUnary(ctx, req)
}

// UpdatePetStatus calls petstore.v1.PetStoreService.UpdatePetStatus.
func (c *petStoreServiceClient) UpdatePetStatus(ctx context.Context, req *connect.Request[v1.UpdatePetStatusRequest]) (*connect.Response[v1.UpdatePetStatusResponse], error) {
	return c.updatePetStatus.CallUnary(ctx, req)
}

// PetStoreServiceHandler is an implementation of the petstore.v1.PetStoreService service.
type PetStoreServiceHandler interface {
	CreatePet(context.Context, *connect.Request[v1.CreatePetRequest]) (*connect.Response[v1.CreatePetResponse], error)
	GetPet(context.Context, *connect.Request[v1.GetPetRequest]) (*connect.Response[v1.GetPetResponse], error)
	ListPets(context.Context, *connect.Request[v1.ListPetsRequest]) (*connect.Response[v1.ListPetsResponse], error)
	UpdatePetStatus(context.Context, *connect.Request[v1.UpdatePetStatusRequest]) (*connect.Response[v1.UpdatePetStatusResponse], error)
}

// NewPetStoreServiceHandler builds an HTTP handler from the service implementation. It returns the
// path on which to mount the handler and the handler itself.
//
// By default, handlers support the Connect, gRPC, and gRPC-Web protocols with the binary Protobuf
// and JSON codecs. They also support gzip compression.
func NewPetStoreServiceHandler(svc PetStoreServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	petStoreServiceMethods := v1.File_petstore_v1_rpc_proto.Services().ByName("PetStoreService").Methods()
	petStoreServiceCreatePetHandler := connect.NewUnaryHandler(
		PetStoreServiceCreatePetProcedure,
		svc.CreatePet,
		connect.WithSchema(petStoreServiceMethods.ByName("CreatePet")),
		connect.WithHandlerOptions(opts...),
	)
	petStoreServiceGetPetHandler := connect.NewUnaryHandler(
		PetStoreServiceGetPetProcedure,
		svc.GetPet,
		connect.WithSchema(petStoreServiceMethods.ByName("GetPet")),
		connect.WithHandlerOptions(opts...),
	)
	petStoreServiceListPetsHandler := connect.NewUnaryHandler(
		PetStoreServiceListPetsProcedure,
		svc.ListPets,
		connect.WithSchema(petStoreServiceMethods.ByName("ListPets")),
		connect.WithHandlerOptions(opts...),
	)
	petStoreServiceUpdatePetStatusHandler := connect.NewUnaryHandler(
		PetStoreServiceUpdatePetStatusProcedure,
		svc.UpdatePetStatus,
		connect.WithSchema(petStoreServiceMethods.ByName("UpdatePetStatus")),
		connect.WithHandlerOptions(opts...),
	)
	return "/petstore.v1.PetStoreService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case PetStoreServiceCreatePetProcedure:
			petStoreServiceCreatePetHandler.ServeHTTP(w, r)
		case PetStoreServiceGetPetProcedure:
			petStoreServiceGetPetHandler.ServeHTTP(w, r)
		case PetStoreServiceListPetsProcedure:
			petStoreServiceListPetsHandler.ServeHTTP(w, r)
		case PetStoreServiceUpdatePetStatusProcedure:
			petStoreServiceUpdatePetStatusHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
}

// UnimplementedPetStoreServiceHandler returns CodeUnimplemented from all methods.
type UnimplementedPetStoreServiceHandler struct{}

func (UnimplementedPetStoreServiceHandler) CreatePet(context.Context, *connect.Request[v1.CreatePetRequest]) (*connect.Response[v1.CreatePetResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("petstore.v1.PetStoreService.CreatePet is not implemented"))
}

func (UnimplementedPetStoreServiceHandler) GetPet(context.Context, *connect.Request[v1.GetPetRequest]) (*connect.Response[v1.GetPetResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("petstore.v1.PetStoreService.GetPet is not implemented"))
}

func (UnimplementedPetStoreServiceHandler) ListPets(context.Context, *connect.Request[v1.ListPetsRequest]) (*connect.Response[v1.ListPetsResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("petstore.v1.PetStoreService.ListPets is not implemented"))
}

func (UnimplementedPetStoreServiceHandler) UpdatePetStatus(context.Context, *connect.Request[v1.UpdatePetStatusRequest]) (*connect.Response[v1.UpdatePetStatusResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("petstore.v1.PetStoreService.UpdatePetStatus is not implemented"))
}
